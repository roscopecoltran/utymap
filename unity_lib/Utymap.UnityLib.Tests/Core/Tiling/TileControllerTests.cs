...
namespace Utymap.UnityLib.Tests.Core.Tiling
{
    [TestFixture]
    public class TileControllerTests
    {
        ...
        public void CanLoadFirstTile()
        {
            ...
            _messageBus.Verify(mb => mb.Send(It.Is<TileLoadStartMessage>(m => CheckQuadKey(m.QuadKey, quadKey))));
        }
        ...
        public void CanLoadNextNorthTile()
        {
            ...
            _messageBus.Verify(mb => mb.Send(It.Is<TileLoadStartMessage>(m => CheckQuadKey(m.QuadKey, newQuadKey))));
        }
